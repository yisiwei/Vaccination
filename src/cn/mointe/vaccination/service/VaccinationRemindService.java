...
public class VaccinationRemindService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (mPreferences.getNotify()) {
			String remindDate = mPreferences.getRemindDate();// 取得提醒日期
			...
			if (!StringUtils.isNullOrEmpty(remindDate)) {
				...
				Log.e("MainActivity", "服务启动...onStartCommand..." + arr[0] + "-"
						+ month + "-" + day);
				int remindTime = mPreferences.getRemindTime();// 取得提醒时间
				...
				Intent remindIntent = new Intent(this,
						VaccinationRemindReceiver.class);
				PendingIntent pendingIntent = PendingIntent.getBroadcast(this,
						0, remindIntent, PendingIntent.FLAG_UPDATE_CURRENT);
				// 30秒后发送广播
				// long sendTime = 30 * 1000;
				// int triggerAtTime = (int) (SystemClock.elapsedRealtime() +
				// sendTime);
				// 只发送一次
